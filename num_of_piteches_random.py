...
Min_Joint_Length = 18.0 # unit m
Max_Joint_Length = 18.3 # unit m
...
with open('listing_centraliser_random.txt', 'w') as f:
    f.write('Total_Pipeline_Length (m): {} \n'.format(Total_Pipeline_Length))
    f.write('Min_Joint_Length (m): {} \n'.format(Min_Joint_Length))
    f.write('Max_Joint_Length (m): {} \n'.format(Max_Joint_Length))
...
while n < Total_Pipeline_Length:
    x = random.uniform(Min_Joint_Length,Max_Joint_Length)
    ranges.append((n+x-Exculsion_Zone, n+x+Exculsion_Zone))
    n += x
    num_of_p += 1
...
with open('listing_centraliser_random.txt', 'a') as f:
